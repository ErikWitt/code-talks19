...
async function execFetch(request) {
    ...
    if (request.url.includes('topProducts')) {
        return cacheThenNetwork(request);
    }

    if (request.url.includes('/v1/connect')) {
...
async function cacheThenNetwork(request) {
    const resp = await cacheOnly(request);
    const networkResponse = networkCacheFallback(request);

    return resp || networkResponse;
}
